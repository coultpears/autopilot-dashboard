...
- **Forward projection now blends booked reservations with the run-rate.** The expanded trend module's "Project forward" toggle previously repeated a flat run-rate (avg of last 3 actual Landing/Partner nets) for every projected month — identical bars, identical tooltips. Now `buildProjectionMonths()` in map.html computes, per projected month, the gross revenue from confirmed reservations overlapping that month (prorated from `property-detail` reservation data — nightly rate×nights or monthly rent by days) and takes the greater of that and the recent 3-month run-rate, split into Landing/Partner net by recent gross→net ratios. Well-booked near months now read above the flat tail; the chart tooltip gains a "Booked so far" line + a "projected" tag so reps see confirmed vs modeled. Run-rate stays the floor (more bookings still come in close-to-date).
